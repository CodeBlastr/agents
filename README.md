...
- Error output includes a text excerpt and screenshot path (default: `/artifacts/taxbot_last_error.png`).
- Most common fixes:
...
### Where to find error screenshots

The screenshot path is generated by the **backend** container (not frontend).

- Default path in-container: `/artifacts/taxbot_last_error.png`
- Compose mounts `./artifacts` on the host to `/artifacts` in backend.
- Host file location: `./artifacts/taxbot_last_error.png`

If you need to inspect manually:

```bash
docker exec -it agents-backend-1 ls -l /artifacts
```
